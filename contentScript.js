...
function onInspectElementClick(a, b, c) {
    ...
    b.push(n);
    b.push(u);
    a = generatePlaywrightSelectors(a);
    console.log(a);
    b.push(a);
    if (isFirefox)
        chrome.runtime.sendMessage({
...
function verifyXpathSelectors(a, b, c, d, e, f) {
    b = [b, c, !0 === d || "true" === d, "" + e];
    if ((_document = _document ? _document : document) && (b || "" === b) && a.includes("highlight-element") && (a = b[1] ? b[1].charAt(0).includes("/") || b[1].charAt(0).includes("(") || b[1].substr(0, 2).includes("./") ? "xpath" : "css" : "xpath",
    b[1]))
        return passResultsToDevtoolsScript(a, b[1], b[2], b[3], f)
}
var passResultsToDevtoolsScript = function(a, b, c, d, e) {
    var f = ""
      , k = ""
      , g = ""
      , l = ""
    ...
    try {
        if (!a.includes("xpath") && "." !== b || elementInShadowDom) {
            m = b;
            try {
                if ("page." != b.trim().slice(0, 5).toLowerCase() && "/" != b.charAt(0)) {
                    var q = _document.querySelectorAll(b);
                    0 == q.length && (m = b.replaceAll('"', "'"),
                    m = 'page.locator("' + m + '")')
                }
            } catch (n) {
                m = b.replaceAll('"', "'"),
            ...
            if ("page." == m.trim().slice(0, 5).toLowerCase() && "/" != b.charAt(0))
                try {
                    if (q = getSelectorMatches(m),
                    q.includes("Invalid Playwright Locator"))
                        return q = "wrongPW Locator errorInfo" + q.split("Invalid Playwright Locator")[1],
                        allNodes = [],
                        p = {
                            errorDetail: q,
                            allNodes,
                ...
                q = _document.querySelectorAll(b);
            !c && elementInShadowDom && (q = inspectedElement.getRootNode().host.shadowRoot.querySelectorAll(b))
        } else {
...
function turnOnDebugger(a) {
    ...
    setTimeout( () => {
        const c = Date.now() - b;
        500 < Math.abs(c - 500) && 600 < Math.abs(c - 500) || chrome.runtime.sendMessage({
